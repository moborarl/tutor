...
4. **เพิ่ม validation เชิง schema ของแต่ละ question type** — ตอนนี้ import validate type หลักแล้ว แต่ยังควรตรวจรูปทรง เช่น `ordering.answer.indices` ต้องครบ ไม่ซ้ำ และอยู่ในช่วงของ `content.items`
5. **เพิ่ม unit tests ให้ `gradeAnswer()`** — อย่างน้อย `fraction` ลดทอนได้ และ `ordering` ถูก/ผิดตาม array
