...
ถ้าโจทย์ข้อไหนต้องดูรูปภาพ/แผนภาพ/กราฟประกอบถึงจะตอบได้ (ไม่ใช่แค่ข้อความเฉยๆ) ให้วาดแผนภาพนั้นขึ้นมาใหม่เป็น SVG เสมอ ใส่ใน field "diagramSvg" — วาดใหม่ทุกครั้งแม้ว่ารูปต้นฉบับที่แนบมาจะมีแผนภาพนั้นชัดเจนอยู่แล้วก็ตาม (เพื่อให้ภาพประกอบทุกข้อมีสไตล์เดียวกัน สวยงามสม่ำเสมอ ไม่ปนกับรูปถ่ายต้นฉบับ)

ข้อกำหนดของ "diagramSvg": ต้องเป็น SVG ที่สมบูรณ์ในตัวเอง เริ่มด้วย <svg viewBox='0 0 400 300' ...> จบด้วย </svg> เสมอ (ห้ามลืมปิด </svg> เด็ดขาด — ถ้าแผนภาพซับซ้อนจนต้องเขียนยาวมากจนอาจตัดจบไม่ทัน ให้วาดแบบง่ายลง ลดจำนวน element ลง แทนที่จะเสี่ยงเขียนไม่จบ), ใช้ shape พื้นฐานเท่าที่จำเป็นเท่านั้น เช่น line/rect/circle/text/polygon/marker สำหรับหัวลูกศร, ห้ามมี <script>, ห้ามอ้างอิงไฟล์ภายนอก, ห้ามใช้ CSS class ภายนอก ให้ใส่ style/fill/stroke แบบ inline เท่านั้น — วาดให้ตรงกับตัวเลข/ทิศทาง/ป้ายกำกับในโจทย์ให้แม่นยำที่สุด เพราะจะใช้แทนรูปต้นฉบับไปเลย

**สำคัญมาก**: ทุก attribute ใน SVG ต้องใช้เครื่องหมาย single quote (') เท่านั้น เช่น <rect x='10' y='20' fill='#333'/> — ห้ามใช้ double quote (") ในแท็ก SVG เด็ดขาด เพราะ diagramSvg ทั้งหมดต้องอยู่ใน string ของ JSON ที่ใช้ double quote ครอบอยู่แล้ว ถ้าใช้ double quote ซ้อนข้างในจะทำให้ JSON พังทันที (ต้อง escape ทุกจุดถ้าใช้ double quote ซึ่งมักพลาด จึงให้ใช้ single quote แทนทั้งหมดเพื่อไม่ต้อง escape เลย)

ข้อที่ไม่ต้องใช้ภาพประกอบ (เป็นแค่ข้อความ) ไม่ต้องใส่ field "diagramSvg"
...
export default function Upload() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // Validate JSON client-side before uploading (tolerating the common mistake
    // of unescaped double quotes inside a "diagramSvg" value).
    const parsed = parseJsonWithSvgRepair(questionsJson) as { questions?: unknown } | null;
